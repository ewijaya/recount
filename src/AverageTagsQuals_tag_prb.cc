// =====================================================================================
// Compute average quality score from common tags with special formatting.
// Input:
// <tag_count>  <tag>  <qA1 qC1 qG1 qT1>  <qA2 qC2 qG2 qT2> etc
//
// This optimized version replaces hardcoded length-specific _tag_prb versions
...
void processBlock(string& tag, vector<vector<double>>& grp) {
    ...
    // Compute and print averages with special formatting
    for (size_t j = 0; j < numCols; j++) {
        sums[j] /= grp.size();
        printf("%.3f", sums[j]);
        cout << " ";
        // Add extra tabs every 4th element (for tag_prb format)
        if (((j + 1) % 4) == 0) {
            cout << "\t\t";
        }
    }
